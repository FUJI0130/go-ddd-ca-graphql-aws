-- ===================================================================
-- ファイル名: aws-test-users.sql
-- 配置場所: scripts/testdata/aws-test-users.sql
-- 説明: AWS環境でのGraphQL認証テスト用ユーザーデータ
-- 
-- 用途:
--  - GraphQL認証機能のテスト用データ投入
...
-- GraphQL認証テスト用マネージャーユーザーの作成（オプション）
INSERT INTO users (id, username, password_hash, role, created_at, updated_at) VALUES 
(
    'USER002',
    'test_manager',
...
-- GraphQL認証テスト用テスターユーザーの作成（オプション）
INSERT INTO users (id, username, password_hash, role, created_at, updated_at) VALUES 
(
    'USER003',
    'test_tester',
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'テストユーザーデータの投入が完了しました';
    RAISE NOTICE '========================================';
    ...
    RAISE NOTICE '2. login mutationでJWTトークンを取得';
    RAISE NOTICE '3. Authorization HeaderにBearer tokenを設定';
