#!/bin/bash
# ===================================================================
# ファイル名: verify-graphql-health.sh
# 説明: GraphQL環境のヘルスチェックを検証するスクリプト
# 
# 用途:
#  - AWS ALBのDNS名を取得
...
echo -e "${BLUE}GraphQL環境のヘルスチェックを検証しています (環境: ${ENVIRONMENT})...${NC}"
...
# ALB_NAME="${ENV_PREFIX}-graphql-alb"
ALB_NAME="${ENV_PREFIX}-graphql${ALB_SUFFIX}-alb"
...
# ターゲットグループのARNを取得
# TG_NAME="${ENV_PREFIX}-graphql-tg"
TG_NAME="${ENV_PREFIX}-graphql${ALB_SUFFIX}-tg"
...
for i in $(seq 1 ${MAX_RETRIES}); do
  ...
  if [ "${HTTP_STATUS}" == "200" ]; then
    ...
    # GraphQL固有のレスポンス検証（JSON形式とステータス確認）
    if command -v jq &>/dev/null; then
      if echo $RESPONSE | jq -e '.status == "ok" and .service == "graphql"' &>/dev/null; then
        echo -e "${GREEN}✓ GraphQLサービス固有のレスポンス形式を確認しました${NC}"
      else
        echo -e "${YELLOW}✗ GraphQLサービス固有のレスポンス形式が正しくありません${NC}"
        echo -e "${YELLOW}  期待: {'status':'ok','service':'graphql'}${NC}"
      fi
    else
      echo -e "${YELLOW}jqコマンドが見つからないため、JSONの詳細検証をスキップします${NC}"
      echo -e "${YELLOW}期待される内容: {'status':'ok','service':'graphql'}${NC}"
    fi
...
# SERVICE_NAME="${ENV_PREFIX}-graphql"
SERVICE_NAME="${ENV_PREFIX}-graphql${ALB_SUFFIX}"
...
# GraphQLエンドポイントの簡易テスト（オプション）
echo -e "${BLUE}GraphQLエンドポイントの簡易テストを実行しています...${NC}"
INTROSPECTION_QUERY='{"query":"{ __schema { queryType { name } } }"}'
GRAPHQL_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d "${INTROSPECTION_QUERY}" http://${ALB_DNS}/query 2>/dev/null || echo "")

if [ ! -z "${GRAPHQL_RESPONSE}" ] && [[ "${GRAPHQL_RESPONSE}" == *"queryType"* ]]; then
  echo -e "${GREEN}✓ GraphQLエンドポイントがIntrospectionクエリに応答しました${NC}"
  echo -e "${BLUE}応答内容 (抜粋):${NC}"
  echo "${GRAPHQL_RESPONSE}" | head -c 100
  echo "..."
else
  echo -e "${YELLOW}警告: GraphQLエンドポイントのテストに失敗しました${NC}"
  echo -e "${YELLOW}ヘルスチェックは成功していますが、GraphQLの機能が完全に動作していない可能性があります${NC}"
fi

echo -e "${GREEN}GraphQL環境のヘルスチェック検証が完了しました${NC}"
exit 0