#!/bin/bash
# ===================================================================
# ファイル名: verify-grpc-health.sh
# 説明: gRPC環境のヘルスチェックを検証するスクリプト
# 
# 用途:
#  - AWS ALBのDNS名を取得
...
echo -e "${BLUE}gRPC環境のヘルスチェックを検証しています (環境: ${ENVIRONMENT})...${NC}"
...
# ターゲットグループのARNを取得
# TG_NAME="${ENV_PREFIX}-grpc-tg"
TG_NAME="${ENV_PREFIX}-grpc-new-tg"  # 実際のHTTP互換ターゲットグループ名を確認要
...
for i in $(seq 1 ${MAX_RETRIES}); do
  ...
  HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://${ALB_DNS}/health-http)
  ...
  if [ "${HTTP_STATUS}" == "200" ]; then
    ...
    RESPONSE=$(curl -s http://${ALB_DNS}/health-http)
    echo $RESPONSE
    
    # gRPC固有のレスポンス検証（JSON形式とステータス確認）
    if command -v jq &>/dev/null; then
      if echo $RESPONSE | jq -e '.status == "ok" and .service == "grpc"' &>/dev/null; then
        echo -e "${GREEN}✓ gRPCサービス固有のレスポンス形式を確認しました${NC}"
      else
        echo -e "${YELLOW}✗ gRPCサービス固有のレスポンス形式が正しくありません${NC}"
        echo -e "${YELLOW}  期待: {'status':'ok','service':'grpc'}${NC}"
      fi
    else
      echo -e "${YELLOW}jqコマンドが見つからないため、JSONの詳細検証をスキップします${NC}"
      echo -e "${YELLOW}期待される内容: {'status':'ok','service':'grpc'}${NC}"
    fi
...
echo -e "${GREEN}gRPC環境のヘルスチェック検証が完了しました${NC}"
exit 0